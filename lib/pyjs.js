...
(function(ns){
    ...
    var arrIndexOf = function(source , match , fromIndex){
    ...
    var prefetchScript = function(urls , cb){
        ...
        var callback = function(url){
            if( arrIndexOf(current_deps , url) != -1 ){
                current_deps.splice(arrIndexOf(current_deps ,url) , 1);
            }
    ...
    var getDependence = function(pkgs){
        ...
        for( var g=0, h=pkgs.length ; g<h; g++ ){
            if( arrIndexOf(final_deps ,pkgs[g]) == -1 ){
                final_deps.push(pkgs[g]);
            }
            for( var c=0,d=t_deps.length ; c<d; c++ ){
                var dep = t_deps[c];
                for( var e=dep.length-1 , f=0 ; e>=f; e-- ){
                    if( arrIndexOf(final_deps , dep[e]) == -1 ){
                        final_deps.unshift(dep[e]);
                    }
                }
            }
        }
