...
class Parser():
    ...
    def parseJs(self , package):
        ...
        if( package == '*' ):
            ...
            self._js[package] = self._js[package] + self.parseCss(package)
    ...
    def parseCss(self , package):
        if package and 'css' in self._manifest and package in self._manifest['css']:
            ...
            if len(css) and 'sys_method' in self._manifest['css']:
                return self._manifest['css']['sys_method'] + '(' + json.dumps(css) + ");"
        return ''
